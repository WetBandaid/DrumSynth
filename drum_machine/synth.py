...
def perc_body(length: int, start_hz: float, end_hz: float, pitch: float, tone_seconds: float) -> np.ndarray:
    t = np.arange(length, dtype=np.float32) / SAMPLE_RATE
    env = timed_envelope(length, max(0.070, tone_seconds * 0.54), 0.0008)
    hit_env = timed_envelope(length, max(0.012, tone_seconds * 0.10), 0.00025)
    ...
    strike = band_limited_noise(length, 2200.0, 9800.0) * hit_env * 0.12
    body = np.tanh((body * env + strike) * 0.92).astype(np.float32)
    return normalize_peak(one_pole_lowpass(body, 5600.0, 0.04), 0.58)
